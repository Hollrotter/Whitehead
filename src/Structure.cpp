...
void Structure::checkMesh()
{
    ...
    {
        std::cout << "Checking mesh of membrane number " << i << '\n';
...
void Structure::principalStresses(const std::string &nxy, const std::string &n12) const
{
    for (size_t k = 0; k < membranes.size(); k++)
        membranes[k]->principalStresses(nxy+"_"+std::to_string(k), n12+"_"+std::to_string(k));
}

void Structure::principalStrains(const std::string &vxy, const std::string &gxy, const std::string &g12) const
{
    for (size_t k = 0; k < membranes.size(); k++)
        membranes[k]->principalStrains(vxy+"_"+std::to_string(k), gxy+"_"+std::to_string(k), g12+"_"+std::to_string(k));
}

void Structure::output(const std::string &filename, const Field field) const
{
    for (size_t k = 0; k < membranes.size(); k++)
