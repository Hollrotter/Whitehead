...
void Airfoil::aerodynamicMatrix()
{
    switch (analysis)
    {
        ...
        case Analysis::nonlinear:
        {
            ...
            nC.zeros(2, nx);
            auto [dxdxi, dzdxi] = chi->derivative(chi->nodes);
            arma::mat D = Lagrange::derivativeMatrix(chi->nodes);
            ...
            for (size_t i = 0; i < nx-1; i++)
            {
                ...
                for (size_t j = 0; j < nx; j++)
                {
                    ...
                    double C_t = dot(A_t.row(i).t(), B_t.row(i).t());
                    arma::vec::fixed<2> c1 = A_t.row(i).t()/A_abs;
                    arma::vec::fixed<2> c2 = B_t.row(i).t()/A_abs - 2*C_t/A4;
                    arma::vec::fixed<2> c3 =-2*C_t/A4*B_t.row(i).t();
                    arma::vec::fixed<2> F_1 = gamma_0 * arma::vec::fixed<2>{c1(1),-c1(0)};
                    arma::vec::fixed<2> F0  = gamma_1 * arma::vec::fixed<2>{c1(1),-c1(0)} + gamma_0 * arma::vec::fixed<2>{c2(1),-c2(0)};
                    arma::vec::fixed<2> F1  = gamma_1 * arma::vec::fixed<2>{c2(1),-c2(0)} + gamma_0 * arma::vec::fixed<2>{c3(1),-c3(0)};
                    ...
                    for (size_t m = 0; m < mx; m++)
                    {
                        ...
                        I_r += F0 + F1*rho + F2*pow(rho, 2);
                    }
                    arma::vec::fixed<2> q = I_r * (xi_max - xi_min)/2;
                    q += F_1*log((1-xi(i))/(1+xi(i)));
                    if (i > 0)
                    {
                        arma::vec::fixed<2> I;
                        for (size_t n = 0; n < nx; n++)
